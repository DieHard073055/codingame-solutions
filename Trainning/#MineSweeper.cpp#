...
char getValue(char **data, int i, int j, int m, int n){
    int value = 0;
    /*Check Top*/
    if(i > 0){if(data[i-1][j] == '*'){value++;}}
    
    /*Check bottom*/
    if(i < n-1){if(data[i+1][j] == '*'){value++;}}
    
    /*Check top Left*/
    if(i > 0 && j > 0){if(data[i-1][j-1] == '*'){value++;}}
    
    /*Check top Right*/
    if(i > 0 && j < m-1){if(data[i-1][j+1] == '*'){value++;}}
    
    /*Check bottom Left*/
    if(i < n-1 && j > 0){if(data[i+1][j-1] == '*'){value++;}}
    
    /*Check bottom Right*/
    if(i < n-1 && j < m-1){if(data[i+1][j+1] == '*'){value++;}}
    
    /*Check left*/
    if(j > 0){if(data[i][j-1] == '*'){value++;}}
    
    /*Check right*/
    if(j < m-1){if(data[i][j+1] == '*'){value++;}}
    
    /*Right the value*/
    if(data[i][j] != '*'){data[i][j] = value +'0';}
}
